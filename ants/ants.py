    ...
    def nearest_bee(self):
        ...
        while not place.is_hive :
            if place.bees and self.lower_bound <= i <= self.upper_bound:
                neareast_place = place
                break
            else:
                place = place.entrance
                i += 1
        if neareast_place:
            return random_bee(neareast_place.bees)
        else:
            return None
        # END Problem 3 and 4
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while loc:
            if loc.ant:
                if loc.ant.is_container and loc.ant.ant_contained:
                    loc.ant.ant_contained.double()
                loc.ant.double()
            loc = loc.exit
        # END Problem 12
